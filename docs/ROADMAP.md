...
- [x] 1.A.3 人脸提取：从媒体中截取人脸照片，生成 face 候选（模型接口走 `agents/llm/` 抽象）（图片走 qwen-vl 真实分析；视频经 ffmpeg 真实抽帧后同链路；人脸比对 match_person_id 仍 stub）
- [x] 1.A.4 语音转写：转写文本 v1 与原始音频双份留存（qwen-audio-turbo 经 dashscope 兼容端点，失败/未配置降级 stub；视频音轨转写 TODO）
...
- [x] 1.C.3 像素小人接入：Package 注册 → 世界中出现对应调色板小人（FR-1.5）（M1.6 完成：confirm → 展位 + 动态实体生成（调色板快照优先/哈希兜底），全链路可点开资料包；per-person 模型属角色管线）
- [x] 1.C.4 场景模块模板：展位/桌位模板 + 现场照片贴图（FR-1.6）（由集市摊位承载：模板含 5 命名展示面，现场照片经相框贴图展示；Blender 程序化生成+校验管线）
...
- [x] 1.D.3 事实层完整性自检：重开页面后资料包与原记录逐字节一致（NFR-1.1 验收）（write_fact 维护 sha256 manifest.v1.json，`GET /api/v0/admin/integrity` 校验端点，篡改/删除检出测试通过）
