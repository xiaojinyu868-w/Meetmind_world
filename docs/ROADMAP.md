...
> 目标（对应 PRD.md §8）：从"一个人的世界"到"一群人可以现场一起玩的世界"。验证：场景能否承载互动？上下文产品如何从零启动？
> 前置：MVP1 验收通过。原 MVP2（Agent 价值发现/推送/微信回填）与原 MVP3（网络与组织）整体暂缓，见文末"暂缓储备"。

### 2.E 场景语言（FR-2.8 / FR-2.9）

- [ ] 2.E.1 点位交互：靠近热点（桌位/吧台/圆桌/摊位）出现 E/F 按键提示，坐下出情境菜单（喝咖啡/邀请人/发起会议）
- [ ] 2.E.2 圆桌会议发起并入场景语言（原 2.A.5 需求并入此条）
- [ ] 2.E.3 世界播报：场景内大屏滚动播报近期世界事件；每日进入时的"晨报"摘要（昨日新相遇与事件）

### 2.F 市集场景（FR-2.10）

- [ ] 2.F.1 市集地图：室外街道布局 + 摊位模块（背景板预留现场照片贴图位，契约见 ART-BRIEF.md）
- [ ] 2.F.2 咖啡厅接入：作为市集街上的一间室内小屋（熟人社交定位，现有咖啡厅场景迁入）
- [ ] 2.F.3 模块规范：摊位/小屋作为应用入口的挂载约定（后续应用可接入）

### 2.G 个人场域（FR-2.11，推断层视觉化）

- [ ] 2.G.1 场域生成管线：个人素材 → AI 生成特质空间（接口先行、mock 可跑；产物标注生成物、可随素材重算，遵守 P-3）
- [ ] 2.G.2 进入他者场域：访问交互（进入 = 相处的空间表达；交互按键化 E/F）

### 2.H 群体冷启动与现场模式（FR-2.12 / 2.13 / 2.14）

- [ ] 2.H.1 合照分割：一张大合照 → AI 分割出每个人 → 批量建档（人脸 → 唯一 person_id）
- [ ] 2.H.2 第一印象互写：自评 1 条 + 每位同伴互评 1 条，入场即带 5–6 条特征（写入推断层，带来源与作者）
- [ ] 2.H.3 现场演示模式：多设备各自操纵 + 大屏全场视角（单机架构内实现；实时联机为远期，见 3.E.1）

### 2.I MVP2 验收走查

- [ ] 2.I.1 现场 ≥5 人合照入场：10 分钟内全部生成形象与特征，世界可浏览
...
- [ ] 3.E.1 联机模式（远期方向登记，2026-08-03，FR-3.6）：现场联机"游戏模式"、云端房间、联机好友注入单机世界；涉及实时同步架构，启动前需架构评审（ARCHITECTURE.md TBD-ARCH-4）
...
- 2026-08-03 | 依 8-03 晚讨论重设 MVP2：新增 2.E 场景语言 / 2.F 市集 / 2.G 个人场域 / 2.H 群体冷启动 / 2.I 验收；原 2.B–2.D（大模型价值分析、推送、微信回填）与 3.A–3.D 整体暂缓入储备区（编号保留）；3.E.1 联机登记为远期方向；1.C.3 人物方向改为 MC 体素 + AI 图片贴图；新增 X.6 上下文量能 feature flag | 人（决策）+ AI（记录）
