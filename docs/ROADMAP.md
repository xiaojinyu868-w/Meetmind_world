...
- [x] 1.C.3 体素小人接入：Package 注册 → 授权头像提取低维可见特征 → 128x128 atlas + 四表情 + 固定身体 GLB → 大厅/咖啡厅动态加载（FR-1.5）
...
- [x] 2.E.1 点位交互：靠近热点（桌位/吧台/圆桌/摊位）出现 E/F 按键提示，坐下出情境菜单（喝咖啡/邀请人/发起会议/调取共同记忆）
- [x] 2.E.2 圆桌会议发起并入场景语言（原 2.A.5 需求并入此条）（热点邀请预选参与者，开始/结束写入世界事件）
- [x] 2.E.3 世界播报：场景内大屏滚动播报近期世界事件；每日进入时的"晨报"摘要（事件 JSONL 持久化，3D 屏与顶部摘要同步刷新）
...
- [x] 2.G.1 场域生成管线：消费上游素材 DTO、第一印象、共同事件与关系记录 → 确定性艺术参数与 4 类互动实体；产物 `echo-field.v1` 标注生成物、来源指针与可重算状态（照片/音频语义提取仍由上游团队负责）
