...
- [ ] M1.2 "人+场景"空间模式（主线）：每个人自带场景模块（展位/摊位/家）作为一级空间；点击进入 TA 的空间；相遇纪念物（相框现场照、录音）= 事实层的空间化呈现；公共咖啡厅降级为广场/集市入口层。影响：快照 modules、导航/视图切换、CafeLayout 或新 Layout
...
- [ ] M1.6 新人入库进世界（1.C.3 的真正完成）：confirm → World Service 注册新 agent → 快照出现该人（调色板/占位模型）→ 世界中可点开其资料包
- [ ] M1.7 PackagePanel `<img>` onerror 降级（破图防护，在面板模块内修）
- [ ] M1.8 座位占用互斥（一座一人，backend world/service.py TODO）
- [ ] M1.9 storybook（v3）场景布局锚点导出（否则活的世界只能锁 v1）
- [ ] M1.10 uvicorn 常驻方式升级：nohup → systemd unit（含开机自启与崩溃重启）
