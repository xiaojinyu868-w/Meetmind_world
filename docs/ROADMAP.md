...
- [x] 2.F.3 模块规范：摊位/小屋作为应用入口的挂载约定（已实现 `echo-scene-app.v1`：booth `app_entry` 只描述应用、目标、状态与能力；具体运行时按 `app_id` 挂载）

### 2.G 个人场域（FR-2.11，推断层视觉化）

- [ ] 2.G.1 场域生成管线：多模态素材（照片/音频语气/第一印象/共同事件/关系状态）→ 情感与关系特征 → 艺术化环境参数映射（空间/天气/声音/实体/互动点），接口先行、mock 可跑；产物标注生成物、可随素材重算（P-3）；映射规则见 TBD-F3（`echo-field.v1` 下游消费契约已就绪；图像/音频/视频到场域的生成链路由独立视觉与音视频工作流负责，当前分支不实现）
- [x] 2.G.2 进入他者场域：访问交互（已实现：资料包/展位应用入口进入全屏关系场域，桌面 WASD + F/Enter、移动端方向键 + 互动按钮；场域明确表达"我与 TA 的关系"并标注为生成物、可重算）

### 2.H 群体冷启动与现场联机（FR-2.12 / 2.13 / 2.14）

- [ ] 2.H.1 合照分割：一张大合照 → AI 分割出每个人 → 批量建档（人脸 → 唯一 person_id）
- [ ] 2.H.2 第一印象互写：自评 1 条 + 每位同伴互评 1 条，入场即带 5–6 条特征（写入推断层，带来源与作者）
- [ ] 2.H.3 现场联机模式：多名现场用户进入同一共享空间，各自设备操纵自己的形象 + 可选大屏全场视角（硬件形态 TBD-H1；实时同步方案需架构评审 TBD-ARCH-4；云端联机为远期 3.E.1）
- [ ] 2.H.4 破冰小游戏：首版选定一个（猜谜/你画我猜/其他，TBD-G1），游戏过程持续采集行为/照片/评价数据

### 2.I MVP2 验收走查

- [ ] 2.I.1 现场 ≥5 人合照入场：10 分钟内全部生成形象与特征，共享空间可浏览
- [ ] 2.I.2 场景语言完整走通一次：点位交互 → 邀请 → 圆桌 → 播报屏可见该事件
- [ ] 2.I.3 破冰互动后数据回流，世界发生可见变化（新特征/新事件上屏）
...
- 2026-08-04 | `echo-scene-app.v1` 展位应用挂载协议 + `echo-field.v1` 下游消费契约落地；资料包可进入全屏关系场域并行走/触发关系节点；2.F.3、2.G.2 完成，2.G.1 多模态生成链路继续由视觉与音视频工作流负责 | 人（分工）+ AI（实现）
