...
| ADR-7 | Agent 只产 Intent，Policy/CommandValidator 后由确定性服务执行 | LLM 不掌握事件顺序、权限、碰撞、会议或游戏状态 |

## 8. TBD

- TBD-ARCH-1：前端是否/何时迁移 TypeScript。
- TBD-ARCH-2：人物数据何时从文件迁移到数据库（触发条件：单人 Package 数 > 500 或多人协作写入冲突出现）。
- TBD-ARCH-3 已决：v0 纯读轮询；MVP2 现场房间使用 WebSocket + sequence cursor replay。
- TBD-ARCH-4：现场联机单节点版本已实现；多实例/云端仍须评审 PostgreSQL outbox、Redis presence/pub-sub、登录与 room token，不能直接扩展单机内存广播。
