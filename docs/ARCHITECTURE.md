...
`backend/app/capabilities/flags.v1.json` 是功能开关的唯一规则源；
`CapabilityService` 读取 Package Store 的已确认数量与推断覆盖率，并接收群体房间
权威 roster 的人数，输出 `echo-capabilities.v1`。前端通过 `CapabilityStore` 消费，
未知能力或快照加载失败一律按关闭处理。

Agent Runtime 和 Hall Runtime 在事件产生前执行同一服务端判断：未满 10 个已确认
Package 时允许个体移动，但禁止访问、交谈和圆桌。`deferred/future` 是独立于数据
条件的发布状态，不能因阈值达标自动开启。视觉组和音频组只向本系统交付版本化
资产引用或上下文 DTO；能力服务不接触原始照片、贴图、音频或视频。
