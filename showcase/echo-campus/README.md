...
项目独立于 EchoWorld 线上业务。本轮入口提供三个真实场地 GLB 预览与校准活动布置；「白庭校园」「水上艺廊」保留为早期程序化场景。活动服务支持演示分身领取、公开供需名片、匹配理由、双方确认相遇与跨设备同步。
...
模型构图使用独立 framingBounds，与活动布置校准的 bounds 分离。入口近景使用 arrival；garden 是建筑细节摄影机位，不表示允许在所有楼层或楼顶行走。
...
1. 在电脑打开在线演示或上方真实场地链接。先选源模型，再切活动布置；拖动环看、滚轮缩放，检查总览、入口、建筑细节和俯瞰。
...
环境声默认关闭，点击右上「声音」后播放轻量入场与连接提示音。当前采用驻足社交呈现：人物落位在已校准的交流点，保留上身待机、点头和一次性招呼，双脚保持站姿；不自动巡游、不播放原地 Walk。桌面 WASD / 方向键平移镜头浏览空间，不再驱动分身；手机仍支持拖动镜头、点选与表单操作。人物位置不是跨设备实时定位。
...
| 能力 | 当前实现 |
| --- | --- |
| 园区场景 | 三个来自源 SKP/3DM 的真实建筑 GLB 与独立活动布置；早期程序化白庭与艺廊保留 |
