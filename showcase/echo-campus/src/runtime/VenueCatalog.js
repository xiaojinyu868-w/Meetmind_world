...
export const VENUE_CANDIDATES = Object.freeze([
  Object.freeze({id:"venue-campus", name:"完整园区", source:"以原始工程坐标组合塔楼、T6主楼、HUB中庭及C地块", archive:"园区原始工程", manifest:"./scenes/venue/venue-campus.json", thumbnail:"./scenes/venue/venue-campus.png", note:"完整园区总览，可切换塔楼、中庭与商业区域。"}),
...
export function venueUrl(id, {baseUrl,view=id==="venue-campus"?"event":"source"}={}) {
...
export function startupVenueFromSearch(search="") {
  ...
  return params.has("scene")||params.has("sceneManifest")?null:"venue-campus";
}

// Shared event links now open the assembled site. Explicit source inspection
// and scope=building keep their original single-building selection.
export function resolveStartupVenue(search = "") {
  const venue = startupVenueFromSearch(search);
  const params = new URLSearchParams(search);
  return venue === "venue-ab-canopy" && params.get("view") === "event" && params.get("scope") !== "building" ? "venue-campus" : venue;
}

// Switching layers on an explicitly opened building must survive a refresh
// or a new shared URL; only the assembled campus has no building scope.
export function venueViewLoadOptions(id, view) {
  if (!venueById(id)) throw new Error("未知的场地源文件");
  return { view: view === "source" ? "source" : "event", ...(id === "venue-campus" ? {} : { scope: "building" }) };
}
