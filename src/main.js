...
import { RoomClient } from "./runtime/RoomClient.js";
...
async function startMeeting(personIds) {
  ...
  let accepted = [];
  if (roomClient) {
    accepted = [...new Set(personIds)].filter((personId) => roomClient.snapshot?.members?.some((m) => m.member_id === personId)).slice(0, 5);
    ...
    await roomClient.send("meeting.invite", {
      ...
      participant_ids: accepted, topic: "最近有什么新变化？",
    });
...
async function endMeeting() {
  ...
  if (roomClient && roomMeetingId) {
    await roomClient.send("meeting.end", { meeting_id: roomMeetingId }).catch((error) => {
