...
然后打开 `http://127.0.0.1:4191/`。服务只监听 `127.0.0.1`，默认会话在内存中隔离，可显式开启独立持久存储，页面可切换查看身份、回放阶段、点击对象、纠正作品标题、逐人接受或拒绝行动、填写本人结果并撤回。“导入一条本人签到”可载入合成示例，或填入最小 JSON；须勾选本人确认并选择可见范围。
导入可重复重试，撤回后不会因旧重试重新出现。重放阶段会重置本会话的实验输入。
服务可能通过 SSH 运行于远程主机，数据默认在实验服务内存中，持久模式保存到显式实验目录，均不写生产数据目录。实验室不会发送消息或报名。默认不会调用模型；启用模型的说明见下一节。
...
此单人入口没有真实认证，查看身份切换只适用于合成实验；独立浏览器加入使用下文的双人凭证入口。持久化需显式启用。

## 保存与恢复世界

页面会记住当前会话、身份和2D/3D模式，刷新后继续。默认服务重启会丢失内存会话；
需要跨重启恢复时，使用独立目录：

~~~bash
backend/.venv/bin/python -m examples.shared_experience.serve --port 4195 --data-dir /root/.local/state/meetmind-lab/user-worlds
~~~

添加 --enable-model 可启用现有模型配置。已成功提交的经历、外观、补丁、计划、选择和结果保存为事件历史。
界面会说明是否支持服务重启恢复；失败时保留原会话标识，不自动覆盖成新世界。
“新建独立实验”保留旧数据，但当前没有旧会话列表或迁移UI。回放阶段仍会重置当前实验，请按按钮说明操作。
普通单人入口的浏览器记录不是登录凭证；双人入口另存角色访问令牌。两者仍是合成实验。详见 [持续世界验收](../../docs/PERSISTENT-LAB-VERIFICATION.md)。

## 两个浏览器分别进入同一世界

独立双人入口按服务端凭证固定alice/bo角色；创建者生成加入链接，另一端明确点击加入后取得bo访问权。
不再通过页面切换来替另一个人选择。各端自动同步自己可见的经历、选择和反馈，创建者可撤销访客访问。
这是合成角色的凭证实验，不是照片本人识别或微信登录，不能把凭证转交当作认领真实身份。

~~~bash
backend/.venv/bin/python -m examples.shared_experience.serve_pair --port 4196 --data-dir /root/.local/state/meetmind-lab/pair-worlds
~~~

在两个隔离浏览器配置中打开本机页面，第一端创建链接，第二端加入。链接仅适用于可访问本机服务的浏览器，
不能直接发给外部手机。界面不会自动发送邀请，双人模式禁用重置共同历史。
访问凭证有效7天、邀请30分钟且只能使用一次。详见 [双端验收](../../docs/PAIR-LAB-VERIFICATION.md)。
...
- 成员列表、actor_id 与 viewer_id **由调用方认证后提供**。纯回放层只校验领域关系，不负责登录或网络授权；
  双人实验由 serve_pair.py 将角色令牌绑定到 viewer 和 session_id。正式产品仍需真实账号及认领流程，
  不能直接把浏览器提交的 actor_id 当成认证结果。
...
目前以纯函数全量回放投影事件，支持经历更正后的依据状态传播与各自重新核对；实验会话支持独立SQLite存储；双人入口提供角色绑定及轮询同步，尚非生产多人服务。
