...
实验尚无真实认证或跨用户加入；持久化需显式启用，当前查看身份切换只适用于合成实验。
...
浏览器记录不是登录凭证；这仍是单机合成实验。详见 [持续世界验收](../../docs/PERSISTENT-LAB-VERIFICATION.md)。
...
目前是纯函数全量回放，实验会话支持可选独立SQLite存储，不是生产事件存储或多人同步服务。现有 RoomService 的持久化、
