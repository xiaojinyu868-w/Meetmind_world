...
$("#panel").addEventListener("click", e => {
  ...
  if (a) {
    ...
    const action = state.actions.find(item => item.id === a.dataset.action);
    if (!action || action.owner !== session.viewer) return;
    if (a.dataset.status === "pending") {
      send({ type: "action.report", action_id: action.id, status: "pending", note: "" });
    } else {
      reportDraft = { action_id: action.id, status: a.dataset.status, session_id: session.session_id };
      $("#report-title").textContent = a.dataset.status === "done" ? "记录本人完成结果" : "记录本人未完成情况";
      $("#report-action-title").textContent = action.text;
      $("#report-error").textContent = "";
      $("#report-note").value = action.report_note || "";
      $("#report-source").value = action.result_source || "";
      $("#report-submit").textContent = a.dataset.status === "done" ? "保存本人完成报告" : "保存本人未完成报告";
...
$("#report-cancel").onclick = () => { $("#report-dialog").close(); };
$("#report-dialog").addEventListener("close", () => { reportDraft = null; });
$("#report-dialog").addEventListener("cancel", e => { if (busy) e.preventDefault(); });
$("#report-form").onsubmit = async e => {
  e.preventDefault();
  if (!reportDraft || busy || session?.session_id !== reportDraft.session_id) return;
  const command = { type: "action.report", action_id: reportDraft.action_id, status: reportDraft.status,
    note: $("#report-note").value, result_source: $("#report-source").value };
  $("#report-submit").disabled = $("#report-cancel").disabled = true;
  try {
    if (await send(command)) $("#report-dialog").close();
    else $("#report-error").textContent = $("#notice").textContent || "结果未保存，请重试。";
  } finally {
